...
$(OUT_DIR)/Torsten\ Uhlmann\ CV\ Summary.tex: $(DATA_DIR)/summary-en.yaml $(TEMPLATE) $(RENDER) | $(OUT_DIR)
	$(PYTHON) $(RENDER) "$(DATA_DIR)/summary-en.yaml" "$(TEMPLATE)" "$@"

$(OUT_DIR)/Torsten\ Uhlmann\ CV\ Übersicht.tex: $(DATA_DIR)/summary-de.yaml $(TEMPLATE) $(RENDER) | $(OUT_DIR)
	$(PYTHON) $(RENDER) "$(DATA_DIR)/summary-de.yaml" "$(TEMPLATE)" "$@"
